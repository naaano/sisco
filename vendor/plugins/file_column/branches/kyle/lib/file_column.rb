...
module FileColumn # :nodoc:
  ...
  #   helper).
  # * <tt>Entry#image</tt>: This will return an absolute path (as a
  #   string) to the currently uploaded file
  #   or nil if no file has been uploaded
  # * <tt>Entry#image_relative_path</tt>: This will return a path relative to
  ...
  module ClassMethods
    ...
    def file_column(attr, options={})
      ...
      define_method "#{attr}_change_extension!" do |*args|
        send(state_method).change_extension! *args
      end

      define_method "#{attr}_url" do |*args|
        send(state_method).url *args
      end
      ...
      FileColumn::Magick::file_column(self, attr, my_options) if options[:magick]
		...
		def validates_extension_of(*attr_names)
      configuration = { :message => ActiveRecord::Errors.default_error_messages[:invalid], :on => :save, :in => DEFAULT_OPTIONS[:extensions] }
      configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)

			configuration[:in] = [].push configuration[:in] if configuration[:in].is_a?(String)

      raise(ArgumentError, "Invalid :in option") unless configuration[:in].is_a?(Array) && configuration[:in].length > 0

			validates_each(attr_names, configuration) do |record, attr_name, value|
				next if value.nil?
				configuration[:coerce] = true if configuration[:coerce].nil?
				ext = record.send(attr_name).scan(/[A-z0-9]+$/).flatten.first
				unless configuration[:in].include?(ext) 
					record.errors.add(attr_name, " has an invalid file type.") unless configuration[:coerce] && record.send("#{attr_name}_state").change_extension!(configuration[:in][0])
				end
      end
		end
		
		def validates_filesize_of(*attr_names)
      configuration = { :message => ActiveRecord::Errors.default_error_messages[:invalid], :on => :save, :in => 0..10.megabytes}
      configuration.update(attr_names.pop) if attr_names.last.is_a?(Hash)
      
      raise(ArgumentError, "Invalid :in option") unless configuration[:in].is_a?(Range)
      
      validates_each(attr_names, configuration) do |record, attr_name, value|
        next if value.nil?
        size = File.size(value)
        record.errors.add(attr_name, " is too small.") unless configuration[:in].first < size 
        record.errors.add(attr_name, " is too large.") unless configuration[:in].last > size 
      end
    end 
  end
