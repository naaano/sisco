...
class FileColumnTest < Test::Unit::TestCase
  ...
  def setup
    ...
    Entry.write_inheritable_attribute :validate, nil
    Movie.write_inheritable_attribute :validate, nil
...
#    FileUtils.rm_rf File.dirname(__FILE__)+"/public"
  ...
  def test_column_read_method
	...
	def test_empty_url_method
		assert Entry.new.respond_to?("image_url")
		assert_equal Entry.new.image_url, nil
	end

	def test_url_method
		e = Entry.new
		e.image = uploaded_file(file_path("kerb.jpg"), "image/jpeg", "local_filename.jpg")
    assert_match %r{/entry/image/tmp/}, e.image_url
	end
  ...
  def test_size_validation
    Entry.validates_filesize_of :image, :in => 5.kilobytes..10.kilobytes
		e = Entry.new
		assert e.save
		e.image = uploaded_file(file_path("kerb.jpg"), "image/jpeg", "local_filename.jpg")
		assert false == e.save, "Too big"
		e.image = uploaded_file(file_path("quote.doc"), "image/png", "quote.doc")
		assert e.save, "Correct validation"
  end

	def test_extension_validation
		Entry.validates_extension_of :image, :in => ["jpg"]
		e = Entry.new
		assert e.save
		e.image = uploaded_file(file_path("kerb.jpg"), "image/jpeg", "local_filename.jpg")
		assert e.save, "Correct validation"
		e.image = uploaded_file(file_path("quote.doc"), "image/png", "quote.doc")
		assert false == e.save, "Intentionally failed validation"
		e.image = uploaded_file(file_path("skanthak.png"), "image/png", "skanthak.png")
		assert e.save, "Validation of transformed image"
	end
	
	def test_multi_validation
  	Entry.validates_extension_of :image, :in => ["xls", "jpg", "jpeg", "abc"], :coerce => false
  	e = Entry.new
  	e.image = uploaded_file(file_path("kerb.jpg"), "image/jpeg", "local_filename.jpg")
		assert e.save, "Correct validation"
		e.image = uploaded_file(file_path("skanthak.png"), "image/png", "skanthak.png")
	...
	def test_validation_no_coerce
		Entry.validates_extension_of :image, :in => ["jpg"], :coerce => false
		e = Entry.new
		e.image = uploaded_file(file_path("skanthak.png"), "image/png", "skanthak.png")
		assert !e.save, "Won't coerce"
	end
	
	def test_mogrify
	  e = Entry.new
	  e.image = uploaded_file(file_path("kerb.jpg"), "image/jpeg", "local_filename.jpg")
	  assert false == e.send("image_state").mogrify(:format => "tuna fish")
	  assert e.send("image_state").mogrify(:format => "gif")
  ...
  def test_assign_with_save
    ...
    assert_equal "#{e.send("image_state").relative_path_prefix}/kerb.jpg", e.image_relative_path
  ...
  def test_dir_methods
    ...
    e.image_dir
    assert_equal File.expand_path(e.send("image_state").relative_path_prefix), File.expand_path(e.image_relative_dir)
  end
